...
| `05_train_test_split.ipynb` | Final train/test split saved to `data/processed/` |

## Steps Performed

1. **Data Collection** – Raw CSV placed in `data/raw/`. Source and collection method documented in `01_eda.ipynb`.
...
6. **Export** – Final processed file written to `data/processed/`.

> **Note:** Do not modify files in `data/raw/`. All transformations are applied in these notebooks and the result is saved to `data/processed/`.
